    ...
    def combine_with_pretrained_model(self, pretrained_model: nn.Module, scaling_coefficient: float = 1.0):
        ...
        with torch.no_grad():
            ...
            for param_name in self.task_vector_param_dict:
                ...
                merged_params[param_name] = pretrained_param_dict[param_name] + scaling_coefficient * self.task_vector_param_dict[param_name].cpu()

        return merged_params

    def dot(self, other):
        """Compute the dot product of two task vectors."""
        with torch.no_grad():
            dot_product = 0
            for param_name in self.task_vector_param_dict:
                dot_product += torch.sum(self.task_vector_param_dict[param_name] * other.task_vector_param_dict[param_name])
        return dot_product
