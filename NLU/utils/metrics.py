...
def compute_metrics(eval_pred: EvalPrediction, dataset_names: list):
    ...
    if len(dataset_names) > 1:
        ...
        for dataset_id in np.unique(dataset_ids):
            ...
            try:
                metric_func = evaluate.load(path='/home/shikexuan/evaluate_metric/glue', config_name=rev_glue_data_id_map[dataset_id.item()])
            except:
    ...
    # single task setting
        ...
        # load the metric function from huggingface or cache
        try:
            metric_func = evaluate.load(path='/home/shikexuan/evaluate_metric/glue', config_name=dataset_name)
        except:
