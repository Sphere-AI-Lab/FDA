...
class GLUEDataLoader:
    ...
    def load_dataset(self, dataset_name: str, val_shot_from_train: int = 64, val_ratio_from_train: float = None, max_seq_length: int = 128, seed: int = 0):
        """
        load GLUE dataset based on dataset_name
        :param dataset_name: str, name of the dataset to load
        :param val_shot_from_train: int, number of examples to use for validation from the training data,
        ...
        :param val_ratio_from_train: float, split ratio of train data for validation, mutually exclusive with "val_shot_from_train"
        ...
        # 这一段注释掉的代码没有意义，因为是好像在线加载。
        # dataset = datasets.load_dataset(path=os.path.join(cache_dir, "glue"), name=dataset_name, storage_options={
        #                                 'client_kwargs': {'timeout': aiohttp.ClientTimeout(total=3600)}})
        dataset = datasets.load_dataset("glue", name=dataset_name,cache_dir='/data/shikexuan/glue_data')
        ...
        #                                                       max_length=max_seq_length, truncation=True),batched=True)
        # padding 128 max seq length
        dataset = dataset.map(lambda examples: self.tokenizer(text=examples[sentence1_key],
                                                              text_pair=examples[sentence2_key] if sentence2_key else None,
                                                              padding="max_length", 
                                                              max_length=max_seq_length, truncation=True),batched=True)
        ...
        permuted_indices = np.random.RandomState(
            seed=seed).permutation(len(dataset["train"])).tolist()
        
        # if val_shot_from_train:
        #     assert val_ratio_from_train is None
        #     num_train_data = len(dataset["train"]) - val_shot_from_train
        # elif val_ratio_from_train:
        #     assert val_shot_from_train is None
        #     num_train_data = int((1 - val_ratio_from_train)
        #                          * len(dataset["train"]))
        # else:
        #     raise ValueError(
        #         "either val_shot_from_train or val_ratio_from_train should be set!")
        num_train_data=len(dataset["train"])
        train_dataset = Subset(
            dataset=dataset["train"], indices=permuted_indices[:num_train_data])
    ...
    def load_multitask_datasets(self, dataset_names: list, train_split_ratio_for_val: float = 0.1, max_seq_length: int = 128):
        ...
        :return:
        """
        assert isinstance(dataset_names, list) and len(
            dataset_names) > 1, f"wrong setting on datasets {dataset_names}!"
        ...
        train_datasets, val_datasets, test_datasets, datasets_num_labels = [dataset[0] for dataset in multiple_datasets], \
            [dataset[1] for dataset in multiple_datasets], [dataset[2]
                                                            for dataset in multiple_datasets], [dataset[3] for dataset in multiple_datasets]
...
class MultiDatasets(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        dataset_index = 0
        assert 0 <= index < len(
            self), f"index {index} out of the length of data {len(self)}!"
